...
describe('convertCbToModelMessages', () => {
  ...
  describe('tool message conversion', () => {
    ...
    it('normalizes circular JSON tool output before provider conversion', () => {
      const value: Record<string, unknown> = { result: 'success' }
      value.self = value
      ...
      expect(JSON.stringify(result)).toContain('"self":"[Circular]"')
    })
