...
function assistantToCodebuffMessage(
  ...
  const content =
    message.content.type === 'tool-call'
      ? {
          ...message.content,
          input: normalizeJsonValue(message.content.input) as Record<
            string,
            unknown
          >,
        }
  ...
  return cloneDeep({ ...message, content: [content] })
}

function convertToolResultMessage(
  message: ToolMessage,
): ModelMessageWithAuxiliaryData[] {
  // Do not copy the outer `content` array into each tool-result part. Besides
  // producing an unnecessary recursive payload, runtime tool values may hold
  // parent/self references and would keep the original circular graph alive
  // even after the actual output value is normalized below.
    ...
    if (c.type === 'json') {
      return cloneDeep<ToolModelMessage>({
        ...
        content: [
          {
            ...messageMetadata,
            output: { ...c, value: normalizeJsonValue(c.value) },
            type: 'tool-result',
          },
        ],
      })
...
function wellFormStringsInPlace(
  value: unknown,
  ...
  for (const key of Object.keys(obj)) {
    ...
    if (typeof item === 'string') {
      ...
      wellFormStringsInPlace(item, visited)
    }
  }
}
