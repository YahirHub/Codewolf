...
#                       non-Codewolf apps, or use --binary)
...
#   -b, --binary [PATH] Use compiled binary (Codewolf-specific shortcut)
#                       If PATH omitted, uses ./cli/bin/codewolf
...
#
#   # Start with default Codewolf dev server (backward compatible)
...
#
#   # Test a compiled binary (Codewolf default location)
#   ./scripts/tmux/tmux-start.sh --binary
#
#   # Test a compiled binary at custom path
#   ./scripts/tmux/tmux-start.sh --binary ./path/to/codewolf
#
# EXIT CODES:
...
WAIT_SECONDS=4
DEFAULT_BINARY="$PROJECT_ROOT/cli/bin/codewolf"
BINARY_PATH="${CODEWOLF_BINARY:-${CODEBUFF_BINARY:-}}"  # Environment variable takes precedence
...
if [[ -n "$CUSTOM_COMMAND" ]]; then
    ...
    # Default mode - Codewolf dev server via bun (for backward compatibility)
