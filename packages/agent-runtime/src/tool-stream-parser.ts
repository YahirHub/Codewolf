...
export async function* processStreamWithTools(params: {
  ...
  async function processToolCallObject(params: {
    ...
    // Tool arguments are part of the persisted message history and the next
    // provider request. Normalize them at the boundary so runtime objects from
    // SDK/provider integrations cannot retain circular references.
