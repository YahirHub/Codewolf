...
export { toTokenCountInputSchema } from './util/tool-schema'
  ...
  if (CACHE_DEBUG_FULL_LOGGING) {
    try {
      cacheDebugCorrelation = createCacheDebugSnapshot({
        ...
        system,
        toolDefinitions: serializeToolDefinitions(params.tools),
...
export async function loopAgentSteps(
  ...
  // where inputSchema is already plain JSON Schema. Anthropic's count_tokens API expects
