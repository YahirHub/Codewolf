...
describe('tool calls', () => {
  ...
  it('serializes circular tool-call inputs without aborting the request', () => {
    const input: Record<string, unknown> = { path: 'src/index.ts' }
    input.parent = input
        ...
        content: [
          {
            type: 'tool-call',
            input: input as any,
            toolCallId: 'call-circular-input',
            toolName: 'read_files',
          },
        ],
      },
  ...
  it('serializes circular JSON tool results without poisoning later turns', () => {
    ...
    const result = convertToOpenAICompatibleChatMessages([
      {
        ...
        content: [
          {
            type: 'tool-result',
            toolCallId: 'call-circular-output',
            toolName: 'custom_tool',
            output: { type: 'json', value: value as any },
          },
        ],
      },
    ])
    ...
    expect(result).toEqual([
      {
        role: 'tool',
        content: JSON.stringify({ ok: true, self: '[Circular]' }),
        tool_call_id: 'call-circular-output',
      },
    ])
  })
})
