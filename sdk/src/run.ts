...
import {
  normalizeJsonValue,
  stringifyJsonValue,
} from '@codebuff/common/util/json'
...
/**
 * Tool integrations are runtime boundaries and can accidentally return class
 * instances or objects with parent/self references. Tool results are persisted
 * and replayed to the model, so normalize JSON outputs once before either path.
 */
function normalizeToolOutputs(outputs: ToolResultOutput[]): ToolResultOutput[] {
  return outputs.map((output) =>
    output.type === 'json'
      ? { ...output, value: normalizeJsonValue(output.value) }
      : output,
  )
}
 ...
 *
 * The safe JSON serializer replaces circular branches with a stable marker and
 * converts bigint values to strings, so snapshots remain persistable instead of
 * retaining a cyclic graph through a cloneDeep fallback.
 */
export function cloneSessionState(
  state: SessionState,
  logger?: Logger,
): SessionState {
  const mainAgentState = JSON.parse(
    stringifyJsonValue(state.mainAgentState),
  ) as SessionState['mainAgentState']
  ...
  if (signal?.aborted) {
    ...
    return {
      sessionState: options.previousRun?.sessionState
        ? cloneSessionState(options.previousRun.sessionState)
        : undefined,
      traceSessionId:
...
async function handleToolCall({
  ...
  if (action.mcpConfig) {
    try {
      ...
      return { output: normalizeToolOutputs(result) }
    } catch (error) {
      return {
        output: [
          {
...
async function handlePromptResponse({
  ...
  if (action.type === 'prompt-error') {
    ...
    const state: RunState = {
      // Never expose the runtime's live mutable graph. Tool implementations
      // may retain Zod/lazy schemas or other objects with parent references;
      // returning that graph makes the very next `previousRun` fail while the
      // SDK clones it. Normalize the final state at the response boundary.
