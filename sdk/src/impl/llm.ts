...
export async function* promptAiSdkStream(
  ...
  const cappedMaxOutputTokens = customProvider?.maxOutputTokens
    ? Math.min(
        streamParams.maxOutputTokens ?? customProvider.maxOutputTokens,
        customProvider.maxOutputTokens,
      )
    : streamParams.maxOutputTokens
  ...
  const response = streamText({
    ...
    // Direct providers must not receive Codebuff/OpenRouter-only provider options.
      ...
      const mainErrorMessage =
        chunkValue.error instanceof Error
          ...
          : typeof chunkValue.error === 'string'
            ? chunkValue.error
            : stringifyJsonValue(chunkValue.error)
...
export async function promptAiSdk(
  ...
  const cappedMaxOutputTokens = customProvider?.maxOutputTokens
    ? Math.min(
        generateParams.maxOutputTokens ?? customProvider.maxOutputTokens,
        customProvider.maxOutputTokens,
      )
    : generateParams.maxOutputTokens
