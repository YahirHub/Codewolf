    ...
    getUserInfoFromApiKey: customProvider
      ? async ({ fields }) =>
          Object.fromEntries(
            fields.map((field) => [
              field,
              field === 'id' ? `local-provider:${customProvider.id}` : null,
            ]),
          ) as any
      : getUserInfoFromApiKey,
    fetchAgentFromDatabase: customProvider
      ? async () => null
      : fetchAgentFromDatabase,
    startAgentRun: customProvider
      ? async () => crypto.randomUUID()
      : startAgentRun,
    finishAgentRun: customProvider ? async () => undefined : finishAgentRun,
    addAgentStep: customProvider
      ? async () => crypto.randomUUID()
      : addAgentStep,
