...
describe('cloneSessionState', () => {
  ...
  it('normalizes circular references into persistable JSON', () => {
    const source = makeSession()
    // Runtime integrations can introduce cycles. Snapshots must stay plain JSON
    // because they are persisted and then used as the next turn's previousRun.
    ...
    expect((clone.mainAgentState as any).output).toEqual({ self: '[Circular]' })

    // Mutating the normalized clone must not affect the source either.
    clone.mainAgentState.messageHistory.push({
      role: 'user',
