...
## Use a custom provider and switch models

The full editor can route the main agent and every subagent directly to an OpenAI-compatible endpoint. Configuration is entirely interactive and does not require provider-management commands outside the editor.

1. Open Codebuff normally and enter `/login`.
2. Provide the provider name, API base URL, and API key. The key may be left blank for local APIs without authentication.
3. Enter model IDs separated by commas, or leave the model field empty and press Enter so Codebuff queries the provider's `/models` endpoint and saves the returned models automatically.
4. Enter `/models` to open the model selector. Models are grouped by provider and the active selection is marked.

The selector also includes the default Codebuff backend, so returning from a custom provider is interactive. The selected provider/model is shown in the status bar.

See [Custom providers and model switching](docs/custom-providers.md) for storage, discovery behavior, and SDK usage.
