...
function isAnalyticsDisabled(env: AnalyticsDeps['env']): boolean {
  const apiKey = env.NEXT_PUBLIC_POSTHOG_API_KEY?.trim().toLowerCase()
  return !apiKey || apiKey === 'disabled' || apiKey.startsWith('disabled-')
}
...
export function initAnalytics() {
  ...
  const apiKey = env.NEXT_PUBLIC_POSTHOG_API_KEY?.trim()

  // Custom-provider-only installations do not need PostHog configuration.
  // The built-in CLI defaults use the explicit `disabled` sentinel.
  if (isAnalyticsDisabled(env)) {
    anonymousId = undefined
    currentUserId = undefined
    client = undefined
    identified = false
  ...
  if (!env.NEXT_PUBLIC_POSTHOG_HOST_URL) {
    const error = new Error('NEXT_PUBLIC_POSTHOG_HOST_URL is not set')
...
export function trackEvent(
  ...
  if (!client) {
    if (isAnalyticsDisabled(resolveDeps().env)) {
      return
    }
    if (isProd) {
...
export function identifyUser(userId: string, properties?: Record<string, any>) {
  const { env, isProd } = resolveDeps()

  // Keep local state coherent while analytics is intentionally disabled.
  if (isAnalyticsDisabled(env)) {
