...
describe('atomic save and resilient load', () => {
  ...
  test('persists circular tool data without aborting the checkpoint', () => {
    const circularInput: Record<string, unknown> = { path: 'src/index.ts' }
    circularInput.self = circularInput
    const messagesWithCycle: ChatMessage[] = [
      {
        id: 'msg-tool',
        variant: 'agent',
        content: '',
    ...
    saveChatState(runState, messagesWithCycle)

    const persisted = JSON.parse(
      fs.readFileSync(path.join(chatDir, 'chat-messages.json'), 'utf8'),
    )
    expect(persisted[0].blocks[0].input).toEqual({
      path: 'src/index.ts',
      self: '[Circular]',
    })
  })
