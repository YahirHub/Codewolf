...
describe('interactive provider commands', () => {
  const createParams = (): RouterParams =>
    ({
      abortControllerRef: { current: null },
      agentMode: 'DEFAULT',
      inputRef: { current: null },
  ...
  test('/login opens the provider wizard without saving prompt history', async () => {
    const command = COMMAND_REGISTRY.find((item) => item.name === 'login')
    const params = createParams()
    const result = await command?.handler(params, '')

    expect(result).toEqual({ openProviderLogin: true })
    expect(params.saveToHistory).not.toHaveBeenCalled()
    expect(params.setInputValue).toHaveBeenCalled()
  })

  test('/models opens the grouped selector without saving prompt history', async () => {
    const command = COMMAND_REGISTRY.find((item) => item.name === 'models')
    const params = createParams()
    const result = await command?.handler(params, '')

    expect(result).toEqual({ openModelSelector: true })
    expect(params.saveToHistory).not.toHaveBeenCalled()
    expect(params.setInputValue).toHaveBeenCalled()
  })
