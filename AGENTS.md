...
- The root package is also installable directly from GitHub with `npm i -g YahirHub/Codewolf`. Keep the npm package dependency-free at runtime, include only `npm/bin`, `npm/lib`, and `npm/postinstall.cjs`, download the matching precompiled release asset during global npm postinstall, and verify it against `SHA256SUMS.txt` before extraction. Normal `bun install` development must not download release binaries.
- The npm installer must select asset names exactly as `.github/workflows/build-binaries.yml` publishes them, detect musl and x64 AVX2/baseline safely, and never fall back from a required baseline build to an AVX2-only build. Keep `tree-sitter.wasm` beside the native executable and reject archive path traversal.
