...
- Configured SSH servers live globally in `~/.codewolf/ssh-servers.json` and remain available across projects and CLI restarts. `list_servers` is the authoritative discovery action; agents must never scrape Codewolf directories to discover configured hosts. Keep `add_server`, `get_server`, `update_server`, `rename_server`, `delete_server`, and `connect_server` explicit and deterministic.
