...
- The main agent must inspect relevant source context before important changes and may create/update numbered `contexto/*.md` records through normal structural file tools. Context files must not contain credentials or secrets.
...
- Verified commits start from a pre-turn Git porcelain baseline and may include only successful `write_file`, `str_replace`, or `apply_patch` mutations that were clean before the turn and still have a final Git change. Never silently include reverted/no-op paths, pre-existing dirty paths, staged work, terminal/script/MCP mutations, paths outside the active Git root, or files changed after the run.
- Pause the message queue while the verification screen is open. A commit requires the explicit `Funciona, crear commit` action. `Necesita correcciones` returns a prefilled correction prompt; skipping leaves disk and Git unchanged. Never push automatically.
